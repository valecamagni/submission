...
- **GEN-SIM step**: the *GEN* step runs hadronization and parton showering (e.g, Pythia), so converts partons from the LHE file into realistic hadronic final states, while the *SIM* step uses Geant4 to simulate how particles interact with the CMS detector (so produces hits in the detctor that mimic what would happen in real data). To run this step you need some ```cmsDriver.py``` commands and a ```GenFragment.py```.
    ...
    The _first_ command processes the LHE events into a ROOT file, making it compatible with the next steps (indeed ```--step NONE```).
    ...
    --era Phase2C17I13M9 
    --python_filename PPD-Phase2Spring24pLHEGS-00002_1_cfg.py 
    --fileout file:PPD-Phase2Spring24pLHEGS-00002_0.root 
    --filein file:PPD-Phase2Spring24pLHEGS-00002_0.lhe
    --no_exec --mc -n 250 || exit $? ;
  ```

    
    
    The _second_ command runs the GEN-SIM simulation, taking the events from the previous step:
    ```
    cmsDriver.py Configuration/GenProduction/python/PPD-Phase2Spring24pLHEGS-00002-fragment.py 
    ...
    --filein file:PPD-Phase2Spring24pLHEGS-00002_0.root
    --no_exec --mc -n 250|| exit $? ;
    ```

    ! In [get setup](https://cms-pdmv-prod.web.cern.ch/mcm/public/restapi/requests/get_setup/PPD-Phase2Spring24pLHEGS-00002), you will find two cmsDriver commands. The first one simply converts the ```.lhe``` file into a ```.root``` file, which is required for the second cmsDriver command. However, you can may skip this step if:
    - you already have a ```.root``` file that contains LHE events
    - you can instead start from a predefined GEN-SIM setup file like:
        - PPD-Phase2Spring24GS-00002.py (skips LHE, starts from GEN-SIM - no ```filein``` option, maybe it's possible just adding it and pointing to the ```.lhe``` file)
        - PPD-Phase2Spring24wmLHEGS-00002.py (does LHE + GEN-SIM in one step, calling and ```externalLHEProducer``` with the specific gridpack)
    
    ! If you want to generate events from scratch, the GenFragment will need to define the physics process explicitly, in addition to configuring the parton showers, hadronization and other simulation settings. Since you are not starting from an LHE file, leave the ```filein``` parameter empty (same would be using the unique command in the setup ```PPD-Phase2Spring24GS-00002.py```)
...
- **Job Submission**: the next step is to combine all configurations and submit jobs for production. This involves:
    1. setting up the CMSSW environment:
        ...
        scram b
        ```
        The GenFragment downloaded here is a common version (i.e. ParticleGan). Remeber to pass the one indicated for you generation.
    
    2. Generate the configs through the ```cmsDriver``` commands of the various steps (described above) in ```CMSSW_14_0_0/src/```

     3. clone the [Submission Repository](https://github.com/cerminar/submission/tree/master): 
        ```
        git clone https://github.com/cerminar/submission.git
        cd submission
        ```

    4. Modify the relevant YAML file: ```submit_GENSIM_DIGIRAW_INFP_140X.yaml```

        ```
        Common:
          mode: 4STEPS   # Defines a 4-step workflow (LHE → GEN-SIM → DIGI-RAW → FastPUPPI inputs)
        ...
        ```
         ...
         - **LHE** (step0.py) → Preliminary step generated from the second cmsDriver command for the GEN-SIM step  
         - **FEVTDEBUG** (step1.py) → First step (GEN-SIM) generated from the second cmsDriver command for the GEN-SIM step.
        ...
        The last part is about _Task-Specific Settings (Dataset m20)_. It uses an LHE input file (```m20.lhe```) which comes from the first LHE Generation step.
        ...
        The ```[Common['mode']]``` will recall the ```run_4STEPS.sh``` script which automates the process of running the four sequential CMSSW steps. It:
        ...
        - runs _step3_ using ```cmsRun``` command and the ```job_config_step3_${PROCID}.py``` configuration file (the output is saved into a compressed log file: ```step3_${PROCID}.${CLUSTERID}.log.gz```)
        - switches to the specific CMSSW version for FastPuppi input slimming step: ```CMSSW_14_2_pre2``` 
     
            ```
            echo 'Switching to CMSSW_14_2_0_pre2 for STEP4...'
            source /cvmfs/cms.cern.ch/cmsset_default.sh
            cd /afs/cern.ch/user/v/vcamagni/FPinputs/CMSSW_14_2_0_pre2
            eval `scram runtime -sh`
            ```
            ! Specify here the path to your FP release.
        - runs _step4_ using ```cmsRun``` command and the ```job_config_step4_${PROCID}.py``` configuration file (the output is saved into a compressed log file: ```step4_${PROCID}.${CLUSTERID}.log.gz```)

        
        
        ! If you don't start from an ```.lhe``` file but directly from the GenFragment, use the 3STEPS common mode, leave the ```input_files: -``` field empty, comment ```process.source.fileNames = cms.untracked.vstring(TEMPL_INFILES)``` in the ```templates/jobCustomization_FEVTDEBUG_cfg.py``` and uncomment:
        ```
